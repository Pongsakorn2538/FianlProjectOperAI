...
seasonal_order = (1, 1, 1, 24)      # (P, D, Q, s) for 24-hour seasonal cycle
...
# Combine date & target for time-series
y_train = df_train.set_index("date_time")[target_vars]
valid_series = df_valid.set_index("date_time")[target_vars]

# Fit SARIMA model
model = sm.tsa.statespace.SARIMAX(y_train,order=order,freq='H',seasonal_order=seasonal_order,enforce_stationarity=False, enforce_invertibility=False,).fit()
...
# Log results
with mlflow.start_run(run_name=f"{model_name}_{target_vars}"):
    mlflow.set_tag("run_type", f"{model_name}_Baseline")
